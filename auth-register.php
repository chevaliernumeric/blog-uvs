...
const ERROR_TO_SHOOT = 'Ce champs est trop court';
const ERROR_PASSWORD_TO_SHOOT = 'Le mot de pass doit faire au moins 6 cractéres';
const ERROR_PASSWORD_MISMATCH = 'Le mot de pass de confirmation est différent';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$firstname) {
    ...
    $errors['firstname'] = ERROR_TO_SHOOT;
  }
  if (!$lastname) {
    $errors['lastname'] = ERROR_REQUIRED;
  } elseif (mb_strlen($lastname) < 2) {
    $errors['lastname'] = ERROR_TO_SHOOT;
  }
  if (!$email) {
  ...
  if (!$password) {
    ...
    $errors['password'] = ERROR_PASSWORD_TO_SHOOT;
  }
  if (!$confirmpassword) {
